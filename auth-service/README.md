...
- ✅ **SQLite Database** - Persistent storage with absolute paths
- ✅ **Comprehensive Tests** - Full pytest suite included
...
This authorization server integrates with the ChatBot service through shared JWT tokens:

1. **Shared Secret Key**: Both services use `AUTH_SECRET_KEY` environment variable
2. **Token Format**: JWT tokens with HS256 algorithm
3. **Token Payload**: Contains `sub` (username), `user_id`, `roles`, and `exp` (expiration)
4. **Network**: Services communicate via Docker network `chatbot-network`
